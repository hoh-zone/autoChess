import { Box, Button, Center, HStack, Img, Input, Modal, Popover, PopoverArrow, PopoverBody, PopoverCloseButton, PopoverContent, PopoverHeader, PopoverTrigger, Spinner, Stack, useToast, VStack } from "@chakra-ui/react"
        ...
        <Center className="h-full w-full relative">
            ...
            <div className="absolute text-white top-12 z-50">
                <Stack className="items-center text-center" gap={4}>
                    <div className="mb-12">
                        <motion.p
                            initial={{ opacity: 0, scale: 0, y: '-3.3vw' }}
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            transition={{ duration: 1 }}
                            style={{ marginBottom: getVw(30), fontSize: getVw(100) }}>Auto<br />Chess</motion.p>
                        <Popover>
                            <PopoverTrigger>
                                        ...
                                        nfts.map((nft, index) => (
                                            <Center className="w-full border-slate-400 mb-1" key={nft.id.id}>
                                                <HStack className="w-full">
                                                    ...
                                                    {nft.arena &&
                                                        <Button className="bg-slate-200" style={{ fontSize: getVw(10) }} onClick={() => openModal(nft.id.id)}>Check Out</Button>
                                                    }
