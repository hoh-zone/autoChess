...
export const Header = () => {
    ...
    return <HStack className="w-full h-[10%] bg-slate-700/50 text-white px-8 absolute z-[100]" spacing={8}>
        ...
        >⬅</Button>
        <Text>{getLocale('Name')}: {name}</Text>
        <Text>{getLocale('Money')}: {money}</Text>
        <Text>{getLocale('Win')}: {win}</Text>
        <Text>{getLocale('Lose')}: {lose}/3</Text>
        {win >= 10 && <Text>{getLocale('ChallengeWin')}: {challengeWin}</Text> }
        {win >= 10 && <Text>{getLocale('ChallengeLose')}: {challengeLose}</Text>}
        <Spacer />
        <Fight />
    </HStack>
}