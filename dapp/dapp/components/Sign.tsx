...
const Sign = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      {signSuccess && <SuccessMessage reset={reset}>{getLocale("Check-the-developer-console-to-see-the-result")}</SuccessMessage>}
      {signError && <ErrorMessage reset={reset}>{getLocale("Signing-did-not-work")}</ErrorMessage>}
      <button className="mx-auto px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700" onClick={sign}>
        {getLocale("Sign-the-message")} &quot;{getLocale("Hello")}&quot;
      </button>
    </div>
  )
}
