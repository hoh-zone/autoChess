...
export const StatusChange = ({ id }: { id: number }) => {
    ...
    <div style={{ zIndex: 1000, position: "relative", justifyContent: "left" }}>
      {hp_change != 0 && (
        <div className="text_animation">
          <p style={{ color: `${get_text_color(hp_change)}` }} className="moving-text">
            {get_hp_text(hp_change)}
          </p>
        </div>
      )}
      {attack_change != 0 && (
        <div className="text_animation">
          <p style={{ color: `${get_text_color(attack_change)}` }} className="moving-text">
            {get_attack_text(attack_change)}
          </p>
        </div>
      )}
    </div>
