...
module chess_packagev3::chess {
    ...
    use challenge_packagev3::challenge;
    ...
    // items are the list of items owned
 ...
 // Operations record the orders made by the play from the interaction with the frontend game portal.
    ...
    // current_items refers to the items owned before the operation
    ...
    // items_str_vec is also frontend result describing the items left owned which needs to be verified
    // after prosessing the operations recorded from frontend and makes sure that it is the same as the pure frontend result
    fun verify_operation(role_global:&role::Global, item_global:&item::ItemsGlobal, current_roles:&mut vector<Role>, cards_pool_roles: &mut vector<Role>, operations: vector<String>,
        ...
        while(vector::length(&operations) > 0) {
            ...
            if (operate == utf8(b"refresh")) {
                ...
                let from_index = 0;
                let to_index = 0;
                let from =  utf8(b"none");
                if(operate == utf8(b"use_chess") || operate == utf8(b"buy_item")|| operate == utf8(b"use_item")){
                    (from, to_index) = utils::get_left_string_right_number(&sub_str);
                }else{
                    (from_index, to_index) = utils::get_left_right_number(sub_str);
                };
                ...
                } else if (operate == utf8(b"use_item")){
                    let item_name = from;
                    let item_use = item::get_item_by_name(item_global, &item_name);
                    let role_apply = vector::borrow_mut<role::Role>(current_roles, to_index);  
                    if(item_name == utf8(b"red_potion")||item_name == utf8(b"purple_potion")||item_name == utf8(b"whet_stone")||item_name == utf8(b"chicken_drumstick")) 
                        item::apply_item_group(current_roles, &item_use)   
                    else    
                        item::apply_item_single(role_apply, &item_use); 

                    //test 
                    print(&utf8(b"use_item: item_name-to_index"));
                    print(&item_name);
                    print(&to_index);
                    //test 
                        
                } else if (operate == utf8(b"use_chess")){
                    let class_name = from;
                    let replace_role = role::get_role_by_class(role_global, class_name);
                    let cur_role = vector::borrow_mut(current_roles, to_index);

                    //test 
                    print(&utf8(b"use_chess: new_class-old_class"));
                    print(&class_name);
                    print(&role::get_class(cur_role));
                    //test 

                    role::set_role(cur_role, &replace_role);
                } else if (operate == utf8(b"buy_item")){
                    let item_name = from;
                    let item_bought = item::get_item_by_name(item_global, &item_name);
                    gold_cost = item::get_item_price(&item_bought);
                    gold = gold - gold_cost;

                    //test 
                    print(&utf8(b"buy_item: item_name-cost"));
                    print(&item_name);
                    print(&gold_cost);
                    //test 
                };
        ...
        //the error is thrown in the check_items_equality function
        // item::check_items_equality(current_items, items_str_vec);
